export const obtenerInicioSemana = (fecha: string): string => {
  if (!fecha) return "";
...
export const obtenerSemana = (fecha: string): number => {
...
export const obtenerAnio = (fila: any): string => {
...
export const obtenerMes = (fila: any): string => {
...
export const getStorageKey = (responsableValor: string): string =>
  `residuos_${(responsableValor || "sin_responsable").trim()}`;

export const getFilaKey = (
  fecha: string,
  responsableFila: string,
  areaId: number | string
): string => `${fecha}__${responsableFila}__${areaId}`;
...
export const getThemeClasses = (modoNoche: boolean) => ({
  ...
  header: modoNoche ? "bg-[#161616] text-gray-200" : "bg-gray-50 text-gray-700",
  fila: modoNoche ? "bg-[#111111] hover:bg-[#1b1b1b]" : "bg-white hover:bg-gray-50",
  borde: modoNoche ? "border-[#303030]" : "border-gray-200",
  linea: modoNoche ? "border-[#3a3a3a]" : "border-gray-200",
  totalGeneral: modoNoche ? "bg-[#1a1a1a] text-gray-200" : "bg-[#f8fafc] text-gray-700",
  chip: modoNoche

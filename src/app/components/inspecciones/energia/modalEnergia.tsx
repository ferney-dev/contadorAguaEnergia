...
import { User, MapPin, Lightbulb, Sun, Calendar, X, ChevronDown } from "lucide-react";
...
const CAMPOS = [
  { key: 1, label: "Bombillas",   icon: <Lightbulb className="w-4 h-4 text-yellow-500" /> },
  { key: 2, label: "Reflectores", icon: <Sun       className="w-4 h-4 text-orange-500" /> },
];

export default function MovilEnergia({ dataBackend = [], setInspecciones, mostrarModal, setMostrarModal, modoNoche }: Props) {
  ...
  const guardar = async () => {
    if (!responsable.trim() || !areaId || !fechaSeleccionada) {
      Swal.fire({ toast: true, position: "top-end", icon: "warning", title: "Debes completar responsable, área y fecha", timer: 1500, showConfirmButton: false });
      return;
    }
    ...
    try {
      const body = {
        ...
        lamparas_c:  0, lamparas_nc:  0,
        aires_c:     0, aires_nc:     0,
        observacion: observacion.trim(),
        total:
          Number(valores?.[1]?.c || 0) + Number(valores?.[1]?.nc || 0) +
          Number(valores?.[2]?.c || 0) + Number(valores?.[2]?.nc || 0),
      };
      ...
      const data = await fetch("/api/inspecciones-energia").then(r => r.json());
      ...
      Swal.fire({ toast: true, position: "top-end", icon: "success", title: "Guardado correctamente", timer: 1400, showConfirmButton: false });
    } catch (error: any) {
      Swal.fire({ icon: "error", title: "Error al guardar", text: error?.message || "Ocurrió un problema" });
    } finally {
      setGuardando(false);
    }
  };
                      ...
                      return (
                        <div
                          ...
                          onMouseDown={(e) => {
                            ...
                            else { setAreasSeleccionadas(p => [...p, a.id]); setAreaId(String(a.id)); setBusquedaArea(a.nombre); }

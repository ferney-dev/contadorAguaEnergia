...
export default function ModalAreasEnergia({ mostrar, setMostrar, modoNoche, areas, setAreas }: Props) {
  ...
  useEffect(() => {
    if (mostrar) {
      setBusqueda(""); setNueva(""); setEditandoId(null);
      setTimeout(() => inputNuevaRef.current?.focus(), 150);
    }
  }, [mostrar]);

  useEffect(() => {
    if (editandoId !== null) setTimeout(() => inputEditRef.current?.focus(), 80);
  }, [editandoId]);
  ...
  const eliminarArea = async (area: Area) => {
    const confirm = await Swal.fire({
      ...
      text: "Se borrarán todas las inspecciones de energía asociadas.",
      icon: "warning",
    ...
    try {
      const res = await fetch(`/api/areas-energia?id=${area.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      ...
      >
        ...
        <div className="bg-[#C40000] px-5 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-bold text-lg leading-tight">Áreas de Energía</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4" style={{ scrollbarWidth: "none" }}>
          ...
          {/* CREAR */}
          <div>
            <p className={`text-[11px] font-bold uppercase tracking-widest mb-2 ${txtSub}`}>Nueva área</p>
            <div className="flex gap-2">
              <div className={`flex-1 flex items-center gap-2 px-3 py-2.5 rounded-2xl border ${inputCls}`}>
                <Plus className="w-4 h-4 text-yellow-500 shrink-0" />
                <input
                  ref={inputNuevaRef}
                  value={nueva}
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-2xl bg-[#C40000] hover:bg-red-700 text-white text-sm font-semibold
                  transition disabled:opacity-40 disabled:cursor-not-allowed shadow-md hover:shadow-red-500/30 active:scale-95 shrink-0"
              >
                {guardando ? "..." : "Crear"}
              </button>
            </div>
                ...
                {areasFiltradas.map((area) => (
                  ...
                  >
                    ...
                    {editandoId === area.id ? (
                      ...
                      <span className={`flex-1 text-sm font-medium truncate ${txtMain}`}>{area.nombre}</span>
                    )}

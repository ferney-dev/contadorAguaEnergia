...
export default function ModalAreasAgua({ mostrar, setMostrar, modoNoche, areas, setAreas }: Props) {
  ...
  const eliminarArea = async (area: Area) => {
    const confirm = await Swal.fire({
      ...
      text: "Se borrarán todas las inspecciones sanitarias asociadas.",
      icon: "warning",
    ...
    try {
      const res = await fetch(`/api/areas-sanitarias?id=${area.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      ...
      >
        ...
        <div className="bg-[#C40000] px-5 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-bold text-lg leading-tight">Áreas Sanitarias</h2>

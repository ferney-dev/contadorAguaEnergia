...
import { User, MapPin, Bath, Droplet, ShowerHead, Toilet, Calendar, X, ChevronDown } from "lucide-react";
...
const CAMPOS = [
  { key: 1, label: "Sanitarios",  icon: <Toilet     className="w-4 h-4 text-blue-500"   /> },
  { key: 2, label: "Orinales",    icon: <Droplet    className="w-4 h-4 text-cyan-500"   /> },
  { key: 3, label: "Duchas",      icon: <ShowerHead className="w-4 h-4 text-indigo-500" /> },
  { key: 4, label: "Lavamanos",   icon: <Bath       className="w-4 h-4 text-pink-500"   /> },
  { key: 5, label: "Llaves",      icon: <Droplet    className="w-4 h-4 text-blue-400"   /> },
];

export default function MovilSanitario({ dataBackend = [], setInspecciones, mostrarModal, setMostrarModal, modoNoche }: Props) {
    ...
    try {
      await fetch("/api/inspecciones-sanitarias", {
        ...
        body: JSON.stringify({
          ...
          sanitarios_c:  Number(valores?.[1]?.c || 0), sanitarios_nc:  Number(valores?.[1]?.nc || 0),
          orinales_c:    Number(valores?.[2]?.c || 0), orinales_nc:    Number(valores?.[2]?.nc || 0),
          duchas_c:      Number(valores?.[3]?.c || 0), duchas_nc:      Number(valores?.[3]?.nc || 0),
          lavamanos_c:   Number(valores?.[4]?.c || 0), lavamanos_nc:   Number(valores?.[4]?.nc || 0),
          llaves_c:      Number(valores?.[5]?.c || 0), llaves_nc:      Number(valores?.[5]?.nc || 0),
          observacion,
        }),
      });
      const data = await fetch("/api/inspecciones-sanitarias").then(r => r.json());
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4" style={{ scrollbarWidth: "none" }}>
          ...
          <div className={`border-t pt-4 ${sep}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {CAMPOS.map((c, idx) => (
                <div key={c.key} className={`rounded-2xl border p-4 ${cardBg} transition hover:shadow-lg ${idx === 4 ? "col-span-2 sm:col-span-1" : ""}`}>

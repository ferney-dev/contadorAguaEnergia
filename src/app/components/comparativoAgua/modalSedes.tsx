...
interface Props {
  ...
  onSedesChange: () => void; // recarga datos completos tras cambio
}

const sedeVacia = () => ({ nombre: "", ubicacion: "", cuenta: "" });

export default function ModalSedes({ mostrar, setMostrar, modoNoche, sedes, setSedes, onSedesChange }: Props) {
  ...
  const eliminarSede = async (sede: Sede) => {
    const confirm = await Swal.fire({
      ...
      text: "Se eliminarán todos los datos de consumo de esta sede.",
      icon: "warning",
    ...
    try {
      // eliminar comparativos de la sede primero
      await fetch(`/api/comparativoAgua/por-sede/${sede.id}`, { method: "DELETE" }).catch(() => {});
  ...
  const itemBg   = modoNoche ? "bg-[#1e1e1e] border-[#2e2e2e] hover:bg-[#252525]" : "bg-white border-gray-200 hover:bg-blue-50/50";
  ...
  return (
    ...
    >
      ...
      >
        {/* ── HEADER ── */}
        <div className="bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-bold text-lg leading-tight">
                {paso === "crear" ? "Nueva Sede / Receptoría" : "Gestión de Sedes"}
              </h2>
              <p className="text-blue-100 text-xs">{sedes.length} sede{sedes.length !== 1 ? "s" : ""} registradas</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4" style={{ scrollbarWidth: "none" }}>
          ...
          {paso === "lista" ? (
            <>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl
                  bg-gradient-to-r from-blue-500 to-cyan-400 text-white font-semibold text-sm
                  shadow-md hover:shadow-blue-400/30 hover:scale-[1.02] active:scale-95 transition"
              >
            ...
            <div className="space-y-3">
              ...
              {/* tips */}
              <div className={`rounded-2xl p-3 text-xs space-y-1 ${modoNoche ? "bg-[#1f1f1f] text-gray-400" : "bg-blue-50 text-blue-600"}`}>
                <p>💡 Ejemplos de nombres:</p>
                <p className="font-semibold">SEDE PPAL BODEGA 4 · RECEPTORIA EL CABLE · RECEPTORIA NORTE</p>
                <p className="mt-1 opacity-80">Los nombres con "SEDE PPAL" y "RECEPTORIA" se agrupan automáticamente en totales.</p>
              </div>
            </div>
          )}
        </div>
          ...
          {paso === "crear" ? (
            <>
              ...
              <button onClick={crearSede} disabled={!nueva.nombre.trim() || guardando}
                className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-blue-500 to-cyan-400 hover:from-blue-600 hover:to-cyan-500

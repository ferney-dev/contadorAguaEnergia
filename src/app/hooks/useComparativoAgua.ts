...
import { FUTURE_YEARS, MESES, START_YEAR } from "../components/comparativoAgua/constants";
import { getThemeClasses, parseNumero } from "../components/comparativoAgua/utils";
...
export type NuevaFila = {
  ...
  ubicacion: string;
  cuenta: string;
  datos: { M3: number; valor: number; cumple: boolean }[];
};
...
export function useComparativoAgua(modoNoche: boolean) {
  ...
  const guardarRegistro = async (fila: FilaComparativo, mesIndex: number) => {
    const sedeId = Number(fila.sede_id || fila.id);
    if (!sedeId) {
    ...
    try {
      ...
      const m3 = parseNumero(mesData.M3);
      const valor = parseNumero(mesData.valor);

      if (m3 === null && valor === null) {
        if (mesData.registroId) {
          const resDel = await fetch(
            `/api/comparativoAgua?id=${mesData.registroId}`,
            { method: "DELETE" }
          );
          if (resDel.ok) {
            setDatosEnergia((prev) =>
  ...
  const calcularTotales = (tipo: string, mesIdx: number): number => {
    return datosEnergia
      ...
      .reduce((acc, d) => {
        ...
        return acc + (d.datos[mesIdx].M3 || 0);
      }, 0);
  };
...
export type UseComparativoAguaReturn = ReturnType<typeof useComparativoAgua>;
